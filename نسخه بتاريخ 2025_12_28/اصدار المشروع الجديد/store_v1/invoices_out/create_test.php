...
");
    ...
    FOR UPDATE");
                ...
                // --- حساب تكلفة البند كما كان ---
                $itemTotalCost = 0.0;
                foreach ($allocations as $a) $itemTotalCost += $a['take'] * $a['unit_cost'];
                $cost_price_per_unit = ($qty > 0) ? ($itemTotalCost / $qty) : 0.0;

                // --- قراءة price_type وتحديد selling_price إن لزم ---
                $price_type = strtolower(trim((string)($it['price_type'] ?? 'retail')));
                if (!in_array($price_type, ['retail', 'wholesale'])) $price_type = 'retail';

                $selling_price = (float)($it['selling_price'] ?? 0);
                // optional: fallback to product prices if selling_price not provided
                if ($selling_price <= 0) {
                    $ppriceStmt = $conn->prepare("SELECT retail_price, wholesale_price FROM products WHERE id = ?");
                    if ($ppriceStmt) {
                        ...
                        $prow = $pres ? $pres->fetch_assoc() : null;
                        if ($prow) {
                            $selling_price = ($price_type === 'wholesale') ? (float)($prow['wholesale_price'] ?? 0) : (float)($prow['retail_price'] ?? 0);
                        }
                        $ppriceStmt->close();
                    }
                }
                ...
                // --- ادراج بند الفاتورة مع price_type ---
   ...
   document.querySelectorAll('.input-qty').forEach(input => {
    ...
    document.querySelectorAll('.fifo-btn').forEach(button => {
        ...
        updatePaymentSection() {
            if (!DOM.paymentTotal || !DOM.paymentPaid || !DOM.paymentRemaining || !DOM.partialPaymentSection) return;
