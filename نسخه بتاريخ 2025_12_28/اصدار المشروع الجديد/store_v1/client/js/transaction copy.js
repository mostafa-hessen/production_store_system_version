...
//     isLoading: false,
//     activeTab: 'wallet', // wallet, returns, statement
    
//     async init(tab = 'wallet') {
//         this.activeTab = tab;
...
//         switch(tab) {
//             case 'wallet':
//                 this.updateWalletTable(transactions);
//                 break;
//             case 'returns':
...
//             const transactionTime = transaction.time || '00:00:00';
...
//     getTabBody(tab) {
//         switch(tab) {
//             case 'wallet': return document.getElementById("walletTableBody");
...
const ApiService = {
  baseUrl: '/api', // تعديل هذا حسب مساراتك

  /**
   * الحصول على حركات العميل
   */
  async getCustomerTransactions(filters = {}) {
    const queryParams = new URLSearchParams(filters).toString();
    const response = await fetch(`${this.baseUrl}/customer_transactions.php?${queryParams}`);
   ...
   * إضافة حركة جديدة
   */
  async addCustomerTransaction(transactionData) {
    const response = await fetch(`${this.baseUrl}/add_customer_transaction.php`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
   ...
   * تصدير الكشف
   */
  async exportCustomerTransactions(filters = {}) {
    const queryParams = new URLSearchParams(filters).toString();
    const response = await fetch(`${this.baseUrl}/export_transactions.php?${queryParams}`);
    return await response.json();
  }
};
