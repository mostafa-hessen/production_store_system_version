...
if (discountAmount > 0) {
    ...
    row.innerHTML = `
        <td>
            <input type="checkbox" class="form-check-input invoice-checkbox print-invoice-checkbox" 
        ...
        <td>
            ...
            <small class="text-muted">
                ${invoice.workOrderName ? `<i class="fa-solid fa-hammer"></i> ${invoice.workOrderName}` : ""}
            </small>
        </td>
...
setupTooltipHover(row, invoiceId) {
            ...
            <div class="amount-with-discount">
                ...
                <div class="discount-badge badge bg-danger">
                    ${discountType === 'percent' ? 
                        `${discountValue}%` : 
                        `${discountAmount.toFixed(2)} ج.م`} خصم
                </div>
            </div>
        `;
        
        // **عرض تفاصيل الخصم في قسم خاص**
        const discountDetailsElement = document.getElementById("invoiceDiscountDetails");
                    ...
                    <div class="card-body p-3">
                        <div class="row">
                            <div class="col-6">
                                ...
                                <div class="fw-bold">${beforeDiscount.toFixed(2)} ج.م</div>
                            </div>
                            <div class="col-6">
                                <small class="text-muted">قيمة الخصم</small>
                                <div class="text-danger fw-bold">-${discountAmount.toFixed(2)} ج.م</div>
                            </div>
                        </div>
                        <div class="row mt-2">
                            <div class="col-6">
                                <small class="text-muted">نوع الخصم</small>
                                <div>${discountType === 'percent' ? 'نسبة مئوية' : 'مبلغ ثابت'}</div>
                            </div>
                            <div class="col-6">
                                <small class="text-muted">${discountType === 'percent' ? 'النسبة' : 'المبلغ'}</small>
                                <div class="fw-bold">
                                    ${discountType === 'percent' ? `${discountValue}%` : `${discountValue} ج.م`}
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    if (invoice.items?.length > 0) {
        // إضافة البنود بدون خصم
        invoice.items.forEach((item) => {
            const row = this.createModalItemRow(item); // بدون تمرير invoice
            tbody.appendChild(row);
        });
        
        // إضافة صف الإجمالي في الأسفل
        const totalRow = document.createElement("tr");
        totalRow.className = "table-active fw-bold";
        
        if (discountAmount > 0) {
            totalRow.innerHTML = `
                <td colspan="3" class="text-end">المبلغ الإجمالي:</td>
                <td colspan="2">
                    <div class="amount-with-discount">
                        <div class="amount-original text-muted text-decoration-line-through">
                            ${beforeDiscount.toFixed(2)} ج.م
