        ...
        <div class="invoice-panel">
            ...
            <div class="add-product-section">
                ...
                <div class="price-type-buttons">
                    <button id="price-wholesale-btn" class="active">جملة</button>
                    <button id="price-retail-btn">قطاعي</button>
                </div>
...
<script>
    ...
    const UI = {
        ...
        updateInvoiceDisplay() {
            ...
            AppState.invoiceItems.forEach((item, index) => {
                ...
                <td>${Helpers.formatCurrency(subtotal)}</td>
                 <td>
        <button class="btn ghost fifo-btn" data-index="${index}">تفاصيل FIFO</button>
    </td>
                <td>
            ...
            document.querySelectorAll('.input-qty').forEach(input => {
                input.addEventListener('change', async function() {
                    ...
                    if (item) {
                        ...
                        // احسب الكمية الإجمالية للمنتج في الفاتورة (بدون البند الحالي)
                        const totalQuantityInInvoice = AppState.invoiceItems
                            .filter((it, idx) => it.id === item.id && idx !== index)
                            .reduce((sum, it) => sum + it.quantity, 0);

                        const newTotal = totalQuantityInInvoice + newQuantity;

                        // احصل على بيانات المنتج من AppData
                        const product = AppData.products.find(p => p.id === item.id);
                        if (product && newTotal > product.remaining_active) {
                            Helpers.showToast(`الكمية الإجمالية (${newTotal}) تتجاوز المخزون المتاح (${product.remaining_active})`, 'error');
                            this.value = item.quantity;
                            return;
                        }

                        // التحقق من المخزون أولاً عبر FIFO
                        const fifoResult = await ApiManager.simulateFIFO(item.id, newQuantity);
                        if (!fifoResult.ok || !fifoResult.sufficient) {
                            const available = fifoResult.allocations.reduce((sum, alloc) => sum + alloc.taken, 0);
                            Helpers.showToast(`الكمية المطلوبة (${newQuantity}) غير متوفرة. المتاح: ${available}`, 'error');
                            this.value = item.quantity;
                            return;
                        }
                        ...
                        UI.update();
                        Helpers.showToast(`تم تحديث كمية ${item.name}`, 'success');
                    }
                });
            });
        ...
        updatePaymentSection() {
            if (!DOM.paymentTotal || !DOM.paymentPaid || !DOM.paymentRemaining) return;
            ...
            if (paymentStatus.value === 'paid') {
                ...
                // إنشاء دفعة تلقائية واحدة بالقيمة الكاملة
                if (AppState.payments.length === 0) {
                    AppState.payments = [{
                        id: Date.now(),
                        amount: total,
                        method: AppState.currentPaymentMethod,
                        notes: AppState.fullPaymentNotes,
                        date: new Date().toLocaleDateString('ar-EG')
                    }];
                } else {
                    // تحديث الدفعة الحالية بالقيمة الكاملة
                    AppState.payments[0] = {
                        id: AppState.payments[0].id,
                        amount: total,
                        method: AppState.currentPaymentMethod,
                        notes: AppState.fullPaymentNotes,
                        date: new Date().toLocaleDateString('ar-EG')
    ...
    `;
        ...
        async processInvoice(shouldPrint = false) {
            ...
            // تحضير بيانات الفاتورة حسب توقعات خوارزمية save_invoice
            const invoiceData = {
                ...
                notes: AppState.invoiceNotes, // يمكن إضافة حقل للملاحظات إذا لزم
                csrf_token: AppState.csrfToken
            };
            console.log('Sending discount data:', {
                type: AppState.discount.type,
                value: AppState.discount.value
            });
        ...
        showSuccessMessage(result) {
            ...
            <div class="invoice-details" style="text-align: right; background: #f8f9fa; padding: 15px; border-radius: 5px; margin: 15px 0;">
                ...
                <div style="border-bottom: 1px dashed #ddd; padding-bottom: 8px; margin-bottom: 8px;">
                    <div>الإجمالي قبل الخصم: <strong>${Helpers.formatCurrency(result.total_before || result.total_revenue)}</strong></div>
                    <div>الخصم: <strong>-${Helpers.formatCurrency(result.discount_amount || 0)}</strong></div>
                    <div>الإجمالي النهائي: <strong style="color: #28a745;">${Helpers.formatCurrency(result.total_after || result.total_revenue)}</strong></div>
                </div>
        `;
    ...
    const InvoiceManager = {
        ...
        //     if (amount > remaining) {
        //         Helpers.showToast('المبلغ المدخل يتجاوز المبلغ المطلوب', 'error');
        //         return;
        //     }
        ...
        //     if (DOM.transferInfo) DOM.transferInfo.value = '';
        // },addPayment
        // إضافة دفعة
        addPayment() {
            const amount = parseFloat(DOM.currentPayment.value) || 0;
        ...
        clearInvoice() {
            if (confirm('هل أنت متأكد من تفريغ جميع بنود الفاتورة وإزالة العميل؟')) {
                ...
                UI.resetInvoice()
                UI.update();
                Helpers.showToast('تم تفريغ بنود الفاتورة وإزالة العميل', 'success');
            }
        },
        ...
        printInvoice(invoiceId = null) {
            const invoiceData = this.prepareInvoiceDataForPrint();
        ...
        prepareInvoiceDataForPrint() {
            ...
            return {
                ...
                invoiceId: AppState.currentInvoiceId || 'طباعه'
            };
        },
    ...
    const EventManager = {
        ...
        //                 if (AppState.currentPaymentMethod === 'bank_transfer') {
        ...
        //     });
        // },
        // في EventManager.setupPaymentEvents، أضف:
        setupPaymentEvents() {
            // تبديل حالة الدفع
            document.querySelectorAll('input[name="payment"]').forEach(radio => {
                radio.addEventListener('change', (e) => {
        ...
        // اختيار عميل
        async selectCustomer(customerId) {
            const result = await ApiManager.selectCustomer(customerId);
            if (result.ok) {
