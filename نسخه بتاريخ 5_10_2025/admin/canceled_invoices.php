...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'fetch_canceled_details') {
    ...
    try {
        ...
        // allocations flat + group by invoice item
//         $allocs = [];
//         $allocs_by_item = [];
//         if ($cancellation && isset($cancellation['id'])) {
//             // join to sale_item_allocations to detect invoice item linkage if present
//             $st4 = $conn->prepare("
//                SELECT 
//     ica.id AS ica_id,
//     ica.cancellation_id,
...
// LEFT JOIN sale_item_allocations sa 
//        ON sa.id = ica.sale_item_allocation_id
// LEFT JOIN invoice_out_items ioi
//        ON ioi.id = sa.sale_item_id 
...
//                 // determine linked invoice item id (several possible column names)
//                 $itemId = null;
//                 if (!empty($r['sa_invoice_out_item_id'])) $itemId = (int)$r['sa_invoice_out_item_id'];
//                 elseif (!empty($r['sa_invoice_item_id'])) $itemId = (int)$r['sa_invoice_item_id'];
//                 elseif (!empty($r['sa_sale_item_id'])) $itemId = (int)$r['sa_sale_item_id'];
//                 else $itemId = null;
...
//                     'qty_restored' => (float)$r['qty_restored'],
//                     'unit_cost' => $r['unit_cost'] !== null ? (float)$r['unit_cost'] : null,
//                     'linked_invoice_item_id' => $itemId,
//                     'product_id' => isset($r['sa_product_id']) ? (int)$r['sa_product_id'] : null
//                 ];
//                 $allocs[] = $alloc;
//                 if ($itemId !== null) {
//                     if (!isset($allocs_by_item[$itemId])) $allocs_by_item[$itemId] = [];
//                     $allocs_by_item[$itemId][] = $alloc;
//                 } else {
//                     if (!isset($allocs_by_item['__unmapped'])) $allocs_by_item['__unmapped'] = [];
//                     $allocs_by_item['__unmapped'][] = $alloc;
//                 }
//             }
//         }
        ...
        <div class="card">
            <div class="d-flex">
                <div class="filters my-4">
                    <input type="text" id="filter_customer_id" placeholder="معرف العميل (Customer ID)">
                    <input type="text" id="filter_invoice_id" placeholder="رقم الفاتورة">
                    <input type="text" id="filter_mobile" placeholder="رقم الهاتف">
                    <button id="btn_filter">بحث</button>
                    <button id="btn_reset" style="background:#6b7280">مسح</button>
                </div>
            </div>
