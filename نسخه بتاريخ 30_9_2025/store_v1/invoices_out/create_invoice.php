...
<div style="margin-top:10px;text-align:right"><button onclick="onId('batchDetailModal_backdrop',el=>el.style.display='none')" class="btn">إغلاق</button></div>
  ...
  <div id="confirmModal_backdrop" style="display:none;position:fixed;inset:0;align-items:center;justify-content:center;background:rgba(0,0,0,0.45);z-index:110">
    <div class="mymodal">
  ...
  <div id="resultModal_backdrop" class="resultModal">
    <div class="mymodal">
...
const CREATED_BY_NAME = "<?php echo $created_by_name_js; ?>"; // قد يكون '' لو لم يُعثر
    ...
    // ---------- showResultModal (enhanced) ----------
    // now accepts rawServerJson as 5th param to map allocation_errors
    // function showResultModal(title, message, success = true, invoiceId = null, rawServerJson = null) {
    //   const modal = $('resultModal_backdrop');
    //   if (!modal) {
    //     console.warn('resultModal_backdrop missing in HTML — fallback to alert');
    //     alert(title + '\n\n' + message);
    //     return;
    //   }

    //   // map allocation_errors -> show names if possible, and disable new-invoice button
    //   if (rawServerJson && rawServerJson.allocation_errors && Array.isArray(rawServerJson.allocation_errors)) {
    //     const mapped = rawServerJson.allocation_errors.map(a => {
    //       // try invoiceItems first, then products
    //       const prod = (Array.isArray(invoiceItems) && invoiceItems.find) ? invoiceItems.find(x => String(x.product_id) === String(a.product_id)) : null;
    //       const pname = prod ? prod.product_name : (products.find ? (products.find(p => String(p.id) === String(a.product_id))?.name) : null);
    //       return (pname ? `${pname}: ${a.msg || a.error || 'مشكلة في التخصيص'}` : (`id:${a.product_id}: ${a.msg || a.error || 'مشكلة في التخصيص'}`));
    //     }).join('\n');
    //     message = mapped;
    ...
    //       createBtn.title = 'معطّل — أصلح أخطاء التخصيص قبل إنشاء فاتورة جديدة';
    //     }
    //   } else {
    //     const createBtn = $('createNewInvoiceBtn');
    //     if (createBtn) {
    ...
    // --- مساعدة بسيطة لاختيار عنصر بالـ id (تفترض أنك تملك $ helper) ---

// تحويل حالة إلى اسم إنساني
// افتراض وجود $ helper
